...
_dag_scheduler = None  # set by main()
...
def affiliate_url(product_url, tag=""):
    """Append Amazon affiliate tag to a product URL, deduping an existing tag."""
    t = tag or os.environ.get("AMAZON_TAG", "")
    if not t:
        return product_url
...
def product_card_html(product, pexels_key="", amazon_tag=""):
    ...
    if asin and price_val:
        bell_svg = '<svg aria-hidden="true" viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8a6 6 0 10-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>'
        alert_btn = (
...
# ── Modern utility-page CSS (comparison + methodology) ───────────────
# These two page types were the last warm-editorial holdouts. They now use
# the same design system + site chrome as every other page type. Legacy
# --text-secondary/--bg-alt/--border aliases are kept for inline styles.
UTILITY_PAGE_CSS = """
""" + DESIGN_SYSTEM_CSS + """
""" + SITE_CHROME_CSS + """
""" + MEGA_MENU_CSS + """
:root {
  --text-secondary: #555; --text-muted: #666; --bg-alt: #f6f5f2;
  --border: #e8e8e8; --bg: #ffffff; --text: #0a0a0a; --primary: var(--clr-primary);
  --primary-dark: #0a0a0a; --accent: var(--clr-accent); --accent-dark: #996015;
}
.hero { background:#0d0d0d; color:#fff; padding:clamp(40px,6vw,72px) 0 56px; position:relative; overflow:hidden; }
.hero::before { content:''; position:absolute; inset:0; background:radial-gradient(900px 420px at 80% 12%, rgba(201,138,44,.14), transparent 65%); pointer-events:none; }
.hero .container { position:relative; }
.hero h1 { color:#fff; margin:0 0 14px; }
.hero p { color:#b9b9b4; max-width:56ch; line-height:1.6; margin:0; font-size:1.05rem; }
.section { padding:clamp(40px,6vw,64px) 0; }
.affiliate-banner { background:#f6f5f2; border:1px solid #e8e8e8; border-radius:var(--radius-sm); padding:14px 20px; font-size:.85rem; color:#666; margin:28px 0; text-align:center; }
.comparison-table { width:100%; border-collapse:collapse; font-size:.92rem; background:#fff; border:1px solid #e8e8e8; border-radius:var(--radius-md); overflow:hidden; }
.comparison-table th { text-align:left; font-family:var(--font-display); font-weight:700; font-size:.78rem; text-transform:uppercase; letter-spacing:.05em; color:#0a0a0a; background:#f6f5f2; padding:12px 14px; border-bottom:1px solid #e8e8e8; }
.comparison-table td { padding:12px 14px; border-top:1px solid #e8e8e8; color:#1a1a1a; vertical-align:top; }
.comparison-table tr:first-child td { border-top:none; }
.comparison-table td small { color:#666; }
.av-score-cell { text-align:center; min-width:84px; }
.av-compact-score { display:block; font-size:1.3rem; font-weight:800; font-family:var(--font-display); color:#0a0a0a; line-height:1; }
.av-compact-label { display:block; font-size:.65rem; color:var(--clr-accent); font-weight:600; text-transform:uppercase; letter-spacing:.04em; margin-top:2px; }
.buy-btn { display:inline-block; padding:8px 16px; background:var(--clr-primary); color:#fff; border-radius:var(--radius-sm); font-weight:700; font-size:.8rem; text-decoration:none; box-shadow:var(--shadow-sm); transition:background var(--duration-fast) var(--ease-out), transform var(--duration-fast) var(--ease-spring); border:none; cursor:pointer; }
.buy-btn:hover { background:var(--clr-accent); transform:translateY(-1px); text-decoration:none; color:#fff; }
.trust-list { list-style:none; display:flex; flex-direction:column; gap:20px; margin:24px 0; }
.trust-item { display:flex; flex-direction:column; gap:2px; padding:18px 20px; background:#f6f5f2; border:1px solid #e8e8e8; border-left:3px solid var(--clr-accent); border-radius:var(--radius-md); }
.trust-item strong { font-family:var(--font-display); color:#0a0a0a; font-size:1rem; }
.trust-item span { color:#555; font-size:.92rem; line-height:1.6; }
.lead-capture { background:#0a0a0a; color:#fff; padding:clamp(40px,6vw,64px) 0; text-align:center; }
.lead-capture h2 { color:#fff; margin:0 0 10px; }
.lead-capture p { color:#b9b9b4; margin:0 0 20px; }
.lead-capture form { display:flex; gap:12px; max-width:480px; margin:0 auto; flex-wrap:wrap; justify-content:center; }
.lead-capture input { padding:12px 16px; border-radius:var(--radius-sm); border:none; font-size:1rem; flex:1; min-width:220px; box-shadow:var(--shadow-sm); color:#0a0a0a; }
.lead-capture button { padding:12px 28px; background:var(--clr-accent); color:#0a0a0a; border:none; border-radius:var(--radius-sm); font-size:1rem; font-weight:700; cursor:pointer; }
.lead-capture button:hover { filter:brightness(1.1); }
"""
...
def build_comparison_page(niche_slug, niche_name, post_title, products, all_slugs, amazon_tag=""):
    ...
    for i, prod in enumerate(products[:6]):
        name = html_mod.escape(clean_product_name(prod.get("name", "Product")))
...
# FONT_LINK and DESIGN_SYSTEM_CSS are imported from src.deployment above —
# single source of truth lives there (run_cycle.py must not carry a duplicate).
...
header{background:#0a0a0a;padding:18px 0;position:sticky;top:0;z-index:100;border-bottom:1px solid #2a2a2a;box-shadow:0 2px 10px rgba(0,0,0,.25)}
...
.hero .featured-pick{display:inline-flex;align-items:center;gap:10px;background:var(--bg);border:1px solid var(--border);border-radius:var(--radius-md);padding:18px 24px;margin-top:16px;box-shadow:var(--shadow-sm);transition:box-shadow .2s,border-color .2s;text-decoration:none;max-width:100%}
...
.cat-card{padding:24px;border:1px solid var(--border);border-radius:var(--radius-md);transition:transform .25s cubic-bezier(.4,0,.2,1),box-shadow .25s cubic-bezier(.4,0,.2,1);box-shadow:var(--shadow-sm);text-decoration:none;display:block;background:var(--bg);position:relative;overflow:hidden}
...
.footer-grid{display:grid;grid-template-columns:1.6fr 2fr 1fr;gap:32px;margin-bottom:32px}
...
.buy-btn{display:inline-block;padding:10px 24px;background:var(--primary);color:#fff;border-radius:8px;font-weight:600;font-size:.95rem;margin-top:8px;text-decoration:none;box-shadow:0 1px 3px rgba(212,99,62,.25);transition:background .2s,box-shadow .2s,transform .2s;border:none;cursor:pointer}
...
.carousel-overlay a{display:inline-flex;align-items:center;gap:8px;color:#fff;font-weight:600;font-size:.95rem;text-decoration:none;padding:14px 32px;background:var(--primary);border-radius:8px;transition:background .2s,transform .2s,box-shadow .2s;width:fit-content;box-shadow:0 2px 12px rgba(212,99,62,.35)}
...
.carousel-dot{width:12px;height:12px;border-radius:50%;background:rgba(255,255,255,.3);border:2px solid rgba(255,255,255,.6);cursor:pointer;transition:background .35s,border-color .35s,transform .35s;padding:0}
.carousel-dot.active{background:#fff;border-color:#fff;transform:scale(1.25)}
.carousel-arrow{position:absolute;top:50%;transform:translateY(-50%);z-index:5;background:rgba(0,0,0,.25);-webkit-backdrop-filter:blur(8px);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.15);color:#fff;width:52px;height:52px;border-radius:50%;cursor:pointer;font-size:1.6rem;display:flex;align-items:center;justify-content:center;transition:opacity .25s,background .25s,transform .25s;opacity:0;font-family:Georgia,serif;line-height:1;padding-bottom:2px}
...
.av-product{font-size:1.2rem;font-weight:800;color:var(--text);font-family:var(--font-display);line-height:1.3;margin:0 0 4px}
.av-breakdown{display:flex;flex-direction:column;gap:8px;margin-bottom:20px}
.av-bar-row{display:flex;align-items:center;gap:12px}
...
.av-summary{font-size:.95rem;color:var(--text-secondary);line-height:1.5;margin-bottom:20px}
.av-cta{display:flex;align-items:center;gap:16px;flex-wrap:wrap}
...
.rps-alt-item{display:flex;align-items:center;gap:12px;padding:12px 14px;border:1px solid var(--border);border-radius:var(--radius-sm);margin-bottom:8px;text-decoration:none;transition:border-color .15s,box-shadow .15s;background:var(--bg-alt)}
...
.rps-reset{background:none;border:1px solid var(--border);border-radius:100px;padding:4px 12px;font-size:.75rem;color:var(--text-muted);cursor:pointer;font-family:inherit;transition:border-color .15s,color .15s}
...
def _archive_row(r, accent, b, niche_slug):
    rel = r.get("rel")
    if not rel:
        rel = f"/reviews/{niche_slug}/"
    href = f'{b.rstrip("/")}{rel if rel.startswith("/") else "/" + rel}'
    title = html_mod.escape(r.get("title", ""))
    date_lbl = ""
    if r.get("updated"):
        try:
            date_lbl = datetime.strptime(r["updated"], "%Y-%m-%d").strftime("%b %d, %Y")
        except ValueError:
            date_lbl = html_mod.escape(r["updated"])
    score_txt = ""
    if r.get("score") and float(r["score"]) <= 10.0:
        score_txt = f'<span class="archive-row__score">{float(r["score"]):.1f}<span class="archive-row__score-out">/10</span></span>'
    return (
        f'<a class="archive-row" style="--cat:{accent}" href="{href}">'
        f'<span class="archive-row__date">{date_lbl}</span>'
        f'<span class="archive-row__title">{title}</span>'
        f'{score_txt}'
        f'<span class="archive-row__arrow" aria-hidden="true">\u2192</span>'
        f'</a>'
    )


def build_category_page(niche_slug, niche_name, reviews, all_slugs, affiliate_tag=""):
    """Niche landing page — dark showroom hero, contents rail, latest reviews,
    full archive index, subscribe band. Mirrors the category listing pages."""
    b = SITE_BASE
    category = next((c for c, slugs in CATEGORY_MAP.items() if niche_slug in slugs), "")
    accent = category_color(category)

    # Newest first; the freshest review becomes the featured spotlight card.
    sorted_reviews = sorted(reviews, key=lambda r: r.get("updated", ""), reverse=True)
    count = len(sorted_reviews)

    # Hero numerics
    valid = [r for r in sorted_reviews if r.get("score") and float(r["score"]) <= 10.0]
    ...
    latest_date = ""
    if sorted_reviews and sorted_reviews[0].get("updated"):
        try:
            latest_date = datetime.strptime(sorted_reviews[0]["updated"], "%Y-%m-%d").strftime("%b %Y")
        except ValueError:
            latest_date = html_mod.escape(sorted_reviews[0]["updated"])

    tagline = NICHE_TAGLINES.get(
        niche_slug,
    ...
    eyebrow = f"{html_mod.escape(category.upper())} · {title_escaped.upper()}"
    if sorted_reviews:
    ...
    <div class="container cat-hero__grid">
        <div class="cat-hero__copy">
            ...
            <h1 class="cat-hero__title">{blog_title}</h1>
            <p class="cat-hero__tagline">{html_mod.escape(tagline)}</p>
            <div class="cat-hero__chips">
                <div class="cat-hero__chip"><span class="cat-hero__chip-num">{count}</span><span class="cat-hero__chip-label">review{"s" if count != 1 else ""} published</span></div>
                <div class="cat-hero__chip"><span class="cat-hero__chip-num"><em>{top_txt}</em>/10</span><span class="cat-hero__chip-label">best verdict</span></div>
                <div class="cat-hero__chip"><span class="cat-hero__chip-num">{latest_date}</span><span class="cat-hero__chip-label">latest update</span></div>
            </div>
    ...
    subscribe_band = (
        ...
        '<p>One email whenever we publish a new guide in this niche. No spam, unsubscribe anytime.</p>'
        '</div>'
    ...
    if sorted_reviews:
        ...
        sections.append(
            ...
            f'<div class="archive-list">{archive_rows}</div></section>'
        )
    else:
        sections.append(subscribe_band)
...
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    <style>
        ...
        .archive-row {{ display:grid; grid-template-columns:130px minmax(0,1fr) 70px 28px; gap: var(--space-md); align-items:center; padding: var(--space-md) var(--space-sm); border-bottom:1px solid var(--clr-light-gray); text-decoration:none; transition: background var(--duration-fast) var(--ease-out); }}
        .archive-row:hover {{ background:var(--clr-off-white); }}
        .archive-row__date {{ font-family:var(--font-mono); font-size:0.78rem; color:var(--clr-mid-gray); letter-spacing:0.03em; }}
        .archive-row__title {{ font-family:var(--font-display); font-weight:600; font-size:1rem; color:var(--clr-black); line-height:1.3; }}
        .archive-row:hover .archive-row__title {{ color: var(--cat, var(--clr-accent-text)); color: color-mix(in srgb, var(--cat, var(--clr-accent-text)) 55%, #1a1200); }}
        .archive-row__score {{ display:inline-flex; align-items:baseline; gap:2px; font-family:var(--font-display); font-weight:800; font-size:1rem; color:var(--clr-black); justify-self:end; }}
        .archive-row__score-out {{ font-size:0.68rem; color:var(--clr-mid-gray); font-weight:600; }}
        .archive-row__arrow {{ font-size:1.1rem; color:var(--clr-mid-gray); justify-self:center; transition: transform var(--duration-fast) var(--ease-out), color var(--duration-fast) var(--ease-out); }}
        .archive-row:hover .archive-row__arrow {{ color: var(--cat, var(--clr-accent)); transform: translateX(3px); }}
        @media (max-width:640px) {{ .archive-row {{ grid-template-columns:1fr auto; }} .archive-row__date {{ grid-column:1 / -1; margin-bottom:2px; }} .archive-row__score {{ justify-self:start; }} }}
...
def _product_aff_url(product, niche_slug, tag):
    """Affiliate URL for a product: real product link, else Amazon search."""
    product_url = (product or {}).get("url", "")
    if product_url:
        return affiliate_url(product_url, tag)
    query = (product or {}).get("affiliate_query") or niche_slug.replace("-", "+")
...
def build_article_page(niche_slug, niche_name, post_title, article_html, intro, product_name, meta_desc, all_slugs, products=None, pexels_key="", amazon_tag="", form_url="", hero_img="", google_client_id="", related_niches=None, published_date=None, updated_date=None, article_id=None, pdf_url=""):
    ...
    matrix_html = (
        ...
        '<h3 class="rail-card__title">At a glance</h3>'
        f'<table class="doc-table"><thead><tr><th scope="col">Use Case</th><th scope="col">Product</th><th scope="col">Why</th></tr></thead>'
    ...
    if products and len(products) > 0:
        ...
        try:
            ...
            verdict_html = render_verdict_card(verdict, p0.get('name', product_name), p0_aff, detail_url, product=p0, category=niche_slug)
        ...
        # Hero visual: the pilot "Our Choice" pick card on the black hero.
        hero_pick = ""
        if products:
            hero_pick = warm_hero_pick_html(p0, verdict_chart_data.get("overall", ""), verdict_chart_data.get("label", ""), p0_aff, b, niche_slug)
        if hero_pick:
            hero_img_html = hero_pick
        elif hero_img:
            hero_img_html = f'<div class="hero-pick__media"><img src="{html_mod.escape(hero_img)}" alt="{html_mod.escape(clean_product_name(p0.get("name", product_name)))}" loading="eager" width="400" height="400"></div>'
        elif p0.get("image"):
            hero_img_html = f'<div class="hero-pick__media"><img src="{html_mod.escape(upgrade_product_image(p0["image"]))}" alt="{html_mod.escape(clean_product_name(p0.get("name", product_name)))}" loading="eager" width="400" height="400"></div>'
    ...
    # Warm chart + matrix sections match the pilot markup exactly.
    chart_html = (
        '<div class="chart-section">'
        '<h3 class="rail-card__title">Performance Breakdown</h3>'
...
def main(forced_niche=None, force=False, batch_mode=False):
    global ai_sql
    secrets = get_secrets()
    ...
    _dag_scheduler.priority_weights = {"kilogateway": 3, "deepseek": 2, "kimi": 2, "groq": 1, "local": 1, "openai": 1}

    # Register cycle change
    change_id = change_mgr.create_change(name="content_cycle_start", change_type=ChangeType.PIPELINE, description="Full content batch cycle")
    ...
    # Close feedback loop after each cycle — pass ai_sql for feedback
    try:
        feedback_result = feedback_loop.close_loop(ai_sql_instance=ai_sql)
